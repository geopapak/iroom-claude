...
$id_sc=$row->ID;
$id_sem=$row->sem;
$sql="SELECT ID FROM exam_programme WHERE ID_day=:id_day AND ID_hour=:id_hour AND ID_user=:id_user AND ID_departament=:id_depart";
$stmt = $dbh->prepare($sql);
...
if($optional=="no"){
	$sql="SELECT exam_programme.ID as id, semester_course.ID_semester as id_sem,ID_course FROM exam_programme INNER JOIN semester_course ON exam_programme.ID_semester_course=semester_course.ID WHERE ID_day=:id_day AND ID_semester_course<>:id_seme AND ID_hour=:id_hour AND semester_course.ID_depart=:id_depart";
	$stmt = $dbh->prepare($sql);
	$stmt->bindParam(':id_day',$int_day,PDO::PARAM_INT);
...
//}
	...
	$sql="SELECT exam_programme_rooms.ID_course FROM exam_programme_rooms  WHERE ID_day_hour=:id_day_hour AND ID_departament=:id_depart ";
	$stmt = $dbh->prepare($sql);
...
if (isset ($_POST['Room'])){
	...
	foreach ($selectedrooms as $value){
		if (isset($value)){
			...
			$sql1="SELECT ID FROM exam_programme_rooms WHERE ID_room=:id_room AND ID_day_hour=:id_day_hour";
...
//var_dump($a);
//die();
if($count2>0 OR $count>0  OR $id_sem==$id_sem2){
	if($count2>0){
		$_SESSION['exist']=10;
		header('Location:exam_calendar.php');
		exit();
	...
	$sql= "INSERT INTO exam_programme(ID_semester_course,ID_day,ID_hour,ID_user,ID_schedule,ID_departament ) VALUES (:id_sc, :id_day, :id_hour, :id_profesor, :year,:id_depart)";
	$stmt = $dbh->prepare($sql);
...
if (isset ($_POST['Room'])){
	...
	foreach ($selectedrooms as $value){
		if (isset($value)){
			$id_room= filter_var( $value, FILTER_SANITIZE_NUMBER_INT);
			$active="active";
			$sql= "INSERT INTO exam_programme_rooms(ID_day_hour,ID_room,ID_course,active,ID_departament) VALUES (:id,:id_room,:id_course,:active,:id_depart)";
			$stmt = $dbh->prepare($sql);
