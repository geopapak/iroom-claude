  ...
  require_once('../connectDB.php');
   ...
   if(isset($_POST["id"])){
       $id_room=$_POST["id"];
        $sql="SELECT * FROM  rooms WHERE ID=:rooms";
		$stmt = $dbh->prepare($sql);
		$stmt->bindParam(':rooms',$id_room,PDO::PARAM_INT);
		$stmt-> execute();
		$row = $stmt ->fetch(PDO::FETCH_OBJ); 
		$room1=$row->name;
$output .= '
<a class="like-button" href="../printarea.php?room='.$id_room.'&nroom='.$room1.'" target="_blank"><i class="fa fa-print fa-2x" aria-hidden="true"></i></a>
<br><div style="text-align: center;font-size: 25px;">Πρόγραμμα εξαμήνου για την αίθουσα "'.$room1.'"</div><br>
...
$output .= 	'<td id="$i.$j" style="text-align:center;  word-break:inherit; width:375px;">';
								while($row1= $stmt1->fetch(PDO::FETCH_OBJ)){
									...
									if($stmt->rowCount()>0){
										$row = $stmt->fetch(PDO::FETCH_OBJ);
										$room=$row->name;
										$output .= ''.$name_course.' ('.$sem.')<br>'.$name_user.' '.$lname_user.'<br>'.$room.'<hr><br>';
									}else{
										$output .='';
									}
								}
