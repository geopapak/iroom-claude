...
if(isset($_SESSION['message'])){
	if($_SESSION['message']==0){
		echo "<div class='alert alert-success' role='alert'>"  ;
		echo "<span class='closebtn1' onclick='this.parentElement.style.display='none';'>&times;</span> ";
		$message = "Έχει σταλθεί με email ο νέος κωδικός.";
		echo "$message</div>";
	}else{
		echo "<div class='alert alert-danger' role='alert'>"  ;
		echo "<span class='closebtn1' onclick='this.parentElement.style.display='none';'>&times;</span> ";
		$message = "Ο κωδικός δεν εχει σταλθεί";
		echo "$message</div>";
	}
...
EOF;
echo $printrooms; 
								$query="SELECT * FROM  users WHERE user_type='Καθηγητής' order by last_name";
								...
								$sql="SELECT ID,name FROM  departament where ID = :depart ";
...
$printrooms = <<< EOF
									...
									<div id="delete$id" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
										...
										<div class="modal-body">
											<p><div style="font-size:larger;color:#b94a48;background-color:#f2dede;border-color:#eed3d7;padding: 8px 35px 8px 14px;margin-bottom: 20px;text-shadow: 0 1px 0 rgba(255,255,255,0.5); border-radius: 4px;opacity: 1;transition: opacity 0.6s;" class="">Σε περίπτωση που τον διαγράψετε θα πρέπει να συμπληρώσετε εκ νέου το πεδίο καθηγητές στα μαθήματα που διδάσκει. Είστε σίγουροι ότι θέλετε να διαγράψετε τον/την <b style="color:red;">$name $last_name</b>? </p>
										</div>
										<hr>
...
$printrooms = <<< EOF
							...
							<a class="btn btn-primary" href="#myModal" data-toggle="modal" style="margin-left: 1%;margin-bottom: 1%;">Προσθήκη Καθηγητή</a> 
							<a class="btn btn-warning" href="#code" data-toggle="modal" style="margin-left: 1%;margin-bottom: 1%;">Προσθήκη/Αλλαγή Κωδικού</a>
							<div style="font-size: 25px;text-align: center;">Πίνακας Καθηγητών</div>
EOF;
echo $printrooms; 
